...
repo_call=$(grep -n '^  ensure_arm_package_repo$' "$ROOT/install/aarch64/install.sh" | cut -d: -f1)
set_call=$(grep -n '^  install_default_package_set$' "$ROOT/install/aarch64/install.sh" | cut -d: -f1)
