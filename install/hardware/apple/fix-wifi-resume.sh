...
# the driver only when the wedge signature actually appears, so it stays a
# no-op on machines and kernels where the firmware bug does not bite - and
# stops acting by itself if the bug is ever fixed.
#
...
if [[ $(uname -m) == "aarch64" ]] && lspci -nn | grep -qE "14e4:(4425|4433)"; then
