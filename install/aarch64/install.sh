...
check_preconditions() {
  ...
  "$checkout/bin/omarchy-hw-aarch64" || fail "This is the Apple Silicon installer. On x86 machines install from the ISO."
  command -v pacman >/dev/null || fail "This installer only supports Arch-based systems."
  command -v sudo >/dev/null || fail "sudo is required."
  "$checkout/bin/omarchy-hw-apple-silicon" ||
...
ensure_arm_package_repo() {
  if ! grep -q '^\[omarchy-aarch64\]' /etc/pacman.conf; then
    ...
    [[ -n $block ]] || fail "default/pacman/aarch64/pacman-stable.conf has no [omarchy-aarch64] section."
